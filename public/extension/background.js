...
async function fetchUserData() {
  try {
    ...
    const profileResponse = await fetch(`${CONFIG.API.BASE_URL}${CONFIG.API.ENDPOINTS.PROFILE}`, {
      headers: {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  console.log('Meeting platform detected:', meetingPlatform, 'for URL:', tab.url);
  ...
  } else if (currentMeeting && currentMeeting.tabId === tabId && !meetingPlatform) {
...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
  if (tabs[0]) {
    ...
    if (meetingPlatform && !currentMeeting) {
      ...
      chrome.tabs.onUpdated.dispatch(tabs[0].id, { status: 'complete' }, tabs[0]);
    }
  }
});
...
function detectMeetingPlatform(url) {
  ...
  if (urlLower.includes('teams.microsoft.com') || urlLower.includes('teams.live.com')) return 'Microsoft Teams';
  return null;
}
...
async function logMeetingSession(sessionData) {
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.API.BASE_URL}${CONFIG.API.ENDPOINTS.CALLS}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${authToken}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to log session: ${errorData.message || response.statusText}`);
    }
...
async function processTranscriptWithAI(transcript) {
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.API.BASE_URL}${CONFIG.API.ENDPOINTS.AI}/suggestions`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${authToken}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Server AI API error: ${errorData.message || response.statusText}`);
    }
    ...
    const validatedSuggestions = {
      ...
      questions: Array.isArray(suggestions?.questions) ? suggestions.questions : [],
      responses: Array.isArray(suggestions?.responses) ? suggestions.responses : [],
      insights: Array.isArray(suggestions?.insights) ? suggestions.insights : []
    };
...
function getFallbackSuggestions() {
  return {
    ...
    questions: ['Could you elaborate on that point?', 'What are your main concerns?'],
...
async function transcribeAudioWithWhisper(audioBlob) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error('Transcription request timed out. Please check your internet connection.');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  console.log('Request data:', { ...request, audioData: request.audioData ? `[${request.audioData.length} chars]` : undefined });
...
async function handleTranscribeAudio(request, sender, sendResponse) {
  ...
  try {
    ...
    // Validate audio data
    if (!audioData || audioData.length < 100) { // Minimum reasonable size for audio data
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.data?.updatedSuggestions || data.updatedSuggestions) {
        const updatedSuggestions = data.data?.updatedSuggestions || data.updatedSuggestions;

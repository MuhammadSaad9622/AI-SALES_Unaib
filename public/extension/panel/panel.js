...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
      ...
      copySuggestionsBtn.addEventListener('click', () => {
        const suggestions = document.getElementById('sales-ai-suggestions').innerText;
    ...
    function updateButtonStates() {
      ...
      toggleTranscriptionBtn.title = isTranscribing ? 'Stop Transcription' : 'Start Transcription';
      toggleSuggestionsBtn.title = isSuggestionsEnabled ? 'Disable Suggestions' : 'Enable Suggestions';
      
      toggleTranscriptionBtn.querySelector('.icon').textContent = isTranscribing ? '🔴' : '🎙️';
    }
    ...
    function updateTranscript(transcript) {
      if (!transcript || !transcript.length) {
        transcriptContainer.innerHTML = '<div class="empty-state">No transcript data available yet.</div>';
        return;
      }
      ...
      transcript.forEach(entry => {
        html += `
          <div class="transcript-line ${entry.speaker ? 'speaker-' + entry.speaker.toLowerCase() : ''}">
    ...
    function updateSuggestions(suggestions) {
      if (!suggestions || Object.keys(suggestions).length === 0) {
        suggestionsContainer.innerHTML = '<div class="empty-state">No suggestions available yet.</div>';
        return;
      }

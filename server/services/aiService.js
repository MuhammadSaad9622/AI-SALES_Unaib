    ...
    try {
      ...
      if (functionCall && functionCall.name === 'generate_sales_suggestion') {
        ...
        return {
          ...
          reasoning: suggestionData.reasoning || 'Based on conversation context',
      ...
      return {
        text: response || "I'm listening to your conversation and will provide suggestions when relevant.",
        type: "general",
        confidence: 0.8,
            ...
            parameters: {
              ...
              properties: {
                key_features: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed descriptions of key product features with specific technical details and capabilities. Include how each feature works and what makes it unique.'
                },
                benefits: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide comprehensive explanations of main benefits with concrete examples of how they solve customer problems. Include quantifiable outcomes when possible.'
                },
                pricing_info: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed pricing information including tiers, subscription models, discounts, and any ROI calculations mentioned in the document.'
                },
                use_cases: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed scenarios and examples of how the product/service is used in real-world situations. Include specific industry applications and customer stories when available.'
                },
                competitive_advantages: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed analysis of competitive advantages with specific comparisons to alternatives in the market. Include unique selling propositions and differentiators.'
                },
                objection_responses: {
                  type: 'array',
                  items: {
                    type: 'object',
                    properties: {
                      objection: { type: 'string', description: 'Detailed description of a potential customer objection' },
                      response: { type: 'string', description: 'Comprehensive, persuasive response that addresses the objection with specific evidence, examples, and counterpoints' }
                    }
                  },
                  description: 'Provide detailed objections that customers might raise and comprehensive responses that address each concern with evidence, examples, and persuasive arguments.'
                },
                target_audience: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed profiles of target audience segments including demographics, psychographics, pain points, and specific needs that the product/service addresses for each segment.'
                },
                success_stories: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Provide detailed customer success stories with specific challenges faced, solutions implemented, and quantifiable results achieved. Include company names and industry context when available.'
                },
                detailed_summary: {
                  type: 'string',
                  description: 'Provide a comprehensive executive summary of the document that captures the most important sales information in a detailed, cohesive narrative.'
                }
              },
  ...
  static async summarize(text) {
    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        model: config.OPENAI_MODEL,
        messages: [
          { role: 'system', content: 'Summarize the following document in a concise paragraph.' },

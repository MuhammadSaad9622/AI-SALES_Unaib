...
router.post('/zoom/sdk-signature', catchAsync(async (req, res) => {
  ...
  try {
    console.log('Generating SDK signature for meeting:', meetingNumber, 'role:', role);
...
router.post('/zoom/send-invite', catchAsync(async (req, res) => {
  const { meetingId, meetingTopic, joinUrl, password, startTime, emails } = req.body;

  if (!meetingId || !emails || !Array.isArray(emails) || emails.length === 0) {
    return res.status(400).json({
      success: false,
...
router.patch('/zoom/:meetingId/disable-notifications', validateObjectId('meetingId'), catchAsync(async (req, res) => {
  const { meetingId } = req.params;
